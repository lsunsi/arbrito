...
impl<'a> Blocks<'a> {
    ...
    pub async fn next(&self) -> Block {
        let (tx, rx) = oneshot::channel();
        self.tx.send(tx).expect("task could not receive request");
        let head = rx.await.expect("next block tx was dropped");
...
async fn task(
    ...
    while open || next.is_some() {
        tokio::select! {
            tx = rx.recv() => match tx {
                Some(tx) => next = Some(tx),
                None => open = false,
            },
            header = stream.next() => match header {
                ...
                Some(head) => {
                    if let Some(tx) = next.take() {
                        ...
                        tx.send(head).expect("next block rx was dropped");
                    }
                }
            }
        }
